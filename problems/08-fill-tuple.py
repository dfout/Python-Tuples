...
def fill_tuple(tup, val, length):
    return tuple(fill_subtuple(subtuple, val, length) for subtuple in tup)



## helper function:

def fill_subtuple(subtup, val, length):
    return subtup + tuple(val for _ in range(length - len(subtup)))
